...
public class PlanillaService {
    ...
    public Integer montoAniosDeServicio(EmpleadoEntity empleado, Integer sueldoFijo){
        ...
        if(anios >= 5 && anios < 10){
            return sueldoFijo*5/100;
        }
        else if (anios >= 10 && anios < 15){
            return sueldoFijo*8/100;
        }
        else if (anios >= 15 && anios < 20){
            return sueldoFijo*11/100;
        }
        else if (anios >= 20 && anios < 25){
            return sueldoFijo*14/100;
        }
        else if (anios >= 25){
            return sueldoFijo*17/100;
        }
        return 0;
    }
    public Integer calcularHorasExtras(EmpleadoEntity empleado){
        Integer horasExtras = dataService.obtenerSumaHorasExtras(empleado.getRut());
        if(horasExtras == null){
            return 0;
        }
    ...
    public Double calcularDescuentos(EmpleadoEntity empleado, Integer sueldoFijo){
        ...
        for(Date fecha : fechas){
            ...
            if(entrada.getHora().compareTo(Time.valueOf("08:10:00")) > 0 && entrada.getHora().compareTo(Time.valueOf("08:25:00")) < 0){
                descuento += sueldoFijo*0.01;
            }
            else if(entrada.getHora().compareTo(Time.valueOf("08:25:00")) > 0 && entrada.getHora().compareTo(Time.valueOf("08:45:00")) < 0){
                descuento += sueldoFijo*0.03;
            }
            else if(entrada.getHora().compareTo(Time.valueOf("08:45:00")) > 0 && entrada.getHora().compareTo(Time.valueOf("09:10:00")) < 0){
                descuento += sueldoFijo*0.06;
            }
            else if(entrada.getHora().compareTo(Time.valueOf("09:10:00")) > 0 && !entrada.getJustificado()){
                descuento += sueldoFijo*0.15;
            }
        }
        return descuento;
    }
